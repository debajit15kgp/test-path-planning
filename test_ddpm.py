...
class CirclePointDataset(Dataset):
    def __init__(self, num_points=10000, radius=1, noise=0.05):
    ...
    def generate_data(self):
        ...
        r = self.radius + np.random.normal(0, self.noise, self.num_points)
        x = r * np.cos(theta)
        y = r * np.sin(theta)
        images = torch.tensor(np.stack([x, y], axis=1), dtype=torch.float32)
        # print(images.shape)
        return images
...
dataset = CirclePointDataset(num_points=config.num_points)
data = dataset.data
...
@torch.no_grad()
def sample_points(model, num_points=10000):
    model.eval()
...
samples = sample_points(trained_model, num_points=1000)
